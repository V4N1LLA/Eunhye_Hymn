...
- 사이드바 토글
  - 데스크톱 헤더/사이드바의 토글 버튼(`<`, `>`)으로 사이드바를 완전히 숨기거나 다시 표시한다.
  - 사이드바 숨김 상태에서도 헤더 토글 버튼으로 즉시 복구할 수 있다.
  - 사이드바는 Soybean 기본 톤에 맞춘 다크 네이비 계열(`bg-[#001529]`)로 고정하고, 활성 메뉴를 보라 포인트로 강조한다.
- 알림 아이콘
  - 헤더 벨 아이콘 클릭 시 운영 점검용 알림 패널이 열린다.
  - 알림 항목 클릭 시 관련 화면(`도움말`, `감사 이벤트`)으로 이동한다.
  - 패널 바깥 클릭 또는 `Esc` 입력 시 패널이 닫힌다.
- 로그인 페이지
  - Soybean 기본 템플릿처럼 웨이브 배경 + 중앙 인증 카드 중심 레이아웃으로 구성한다.
  - 카드 상단 브랜딩/상태 액션, 본문 인증 폼, 하단 보조 액션(세션 초기화/도움말) 계층을 분리한다.
  - 기존 로그인 문구 및 인증 플로우(ID/PW 로그인, 세션 초기화, 로그인 도움말, 로컬 개발 로그인)는 유지한다.
