...
- Admin UI/UX 개선 (Soybean 스타일 보강)
  ...
  - 로그인 페이지: 운영 안내/인증 영역 분리 + wave 배경/글래스 카드 레이어 적용(기존 인증 문구/플로우 유지)
  - 2차 시각 정렬: 다크 네이비 사이드바 + 화이트 헤더 구조로 템플릿 톤 재정렬, 로그인은 중앙 인증 카드 중심 레이아웃으로 재구성
  - 대시보드 톤 보강: 상단 인사/요약 영역과 KPI 카드 밀도를 Soybean 템플릿 감성에 맞춰 정리
- 문서 동기화
