...
class AuthAiOpsAlertSchedulerTest {
    @Test
    void evaluateSnapshotUsesDeltaAndAggregatesOnlyConfiguredAuthScopes() {
        SimpleMeterRegistry meterRegistry = new SimpleMeterRegistry();
        meterRegistry.counter("auth_requests_total", "scope", "user_login", "result", "success").increment(10.0);
        meterRegistry.counter("auth_requests_total", "scope", "user_login", "result", "failure").increment(4.0);
        meterRegistry.counter("auth_requests_total", "scope", "social_login", "result", "success").increment(1.0);
        meterRegistry.counter("auth_requests_total", "scope", "social_login", "result", "failure").increment(1.0);
        ...
        AuthAiOpsAlertScheduler.OpsSnapshot warmupSnapshot = scheduler.evaluateSnapshot();
        assertThat(warmupSnapshot.authFailure().sampleReady()).isFalse();
        assertThat(warmupSnapshot.authFailure().breached()).isFalse();
        ...
        meterRegistry.counter("auth_requests_total", "scope", "admin_login", "result", "failure").increment(12.0);
    ...
    void evaluateSnapshotFlagsAiFailureAndFallbackThresholdBreaches() {
        ...
        AuthAiOpsAlertScheduler.OpsSnapshot warmupSnapshot = scheduler.evaluateSnapshot();
        assertThat(warmupSnapshot.aiFailure().sampleReady()).isFalse();
        assertThat(warmupSnapshot.aiFallback().sampleReady()).isFalse();
